...
const COUNTY_SLUG = "orange-county";
...
export default async function OrangeCountyCityPage({ params }: PageProps) {
  const { city: citySlug } = await params;
  const row = getCity(COUNTY_SLUG, citySlug);
  if (!row) notFound();
  ...
  const fallback = {
    ...
    sections: [
      {
        ...
        body: [
          `${row.city} offers a wonderful combination of community, schools, and lifestyle for growing families in Orange County.`,
    ...
    faqs: [
      { q: `Is ${row.city} good for families?`, a: `${row.city} is known for its family-friendly environment, good schools, and community amenities. It's a popular choice for growing families in Orange County.` },
      ...
      { q: `What school districts serve ${row.city}?`, a: `${row.city} is served by highly-rated Orange County school districts. We can provide specific school zone information during your consultation.` },
    ],
