...
export default async function SanDiegoCityPage({ params }: PageProps) {
  const { city: citySlug } = await params;
  const row = getCity(COUNTY_SLUG, citySlug);
  if (!row) notFound();
  ...
  // Fallback content when no stored JSON exists
  const fallback = {
    ...
    sections: [
      {
        ...
        body: [
          `${row.city} offers a wonderful combination of community, schools, and lifestyle for growing families in Southern California.`,
